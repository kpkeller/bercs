...
parameters {
    ...
    // vector<lower=if_else(beta_lower_lim==negative_infinity(), negative_infinity(), (beta_lower_lim - prior_beta_mean)/sigmaBeta)>[xdf] beta_raw; // Coefficients for exposure spline
    vector[n] reI_raw;
}
transformed parameters {
    ...
    mui= bS[study_of_obs]  + Hx*beta + reI[subj_of_obs]; 
    if (p >0 ){
